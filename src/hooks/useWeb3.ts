...
export function useWeb3() {
  ...
  const [psgTokenBalance, setPsgTokenBalance] =
    useState<FanTokenBalance | null>(null);
  ...
  const getPsgTokenBalance = useCallback(
    async (
      userAddress: string,
      useSimulation: boolean = true
    ): Promise<FanTokenBalance> => {
      if (!userAddress) {
        return {
          balance: BigInt(0),
          decimals: 18,
      ...
      if (useSimulation) {
        const hasTokens = SIMULATED_TOKEN_HOLDERS.has(
  ...
  const hasEnoughPsgTokens = useCallback(() => {
    ...
    return (
      psgTokenBalance.hasTokens &&
      psgTokenBalance.balance >= psgTokenBalance.minimumForDiscount
    );
  }, [psgTokenBalance]);
